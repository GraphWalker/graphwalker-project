		...
		_bind: function() {
			...
			if (!options.disablePan || !options.disableZoom) {
				events[ str_start ] = function(e) {
					...
					if (e.type === 'touchstart' ?
						...
						!options.disablePan && e.which === 3) { // STUDIO EDIT: only trigger on right click!
			...
			if ($zoomIn.length && $zoomOut.length) {
				...
				$zoomIn.on(str_click, function(e) {
					e.preventDefault();
					self.zoom({focal: {
						clientX: (self.$parent[0].clientWidth + self.elem.clientWidth)/2,
						clientY: (self.$parent[0].clientHeight + self.elem.clientHeight)/2
					}});
				});
				$zoomOut.on(str_click, function(e) {
					e.preventDefault();
					self.zoom(true, {focal: {
						clientX: (self.$parent[0].clientWidth + self.elem.clientWidth)/2,
						clientY: (self.$parent[0].clientHeight + self.elem.clientHeight)/2
					}});
				});
			...
			if ($zoomRange.length) {
				...
				events[ (supportsInputEvent ? 'input' : 'change') + ns ] = function() {
					// STUDIO: set focal point to center
					var parent = self.$parent[0];
					self.zoom(+this.value, { noSetRange: true, focal: {
						clientX: (parent.clientWidth + self.elem.clientWidth)/2 + parent.offsetLeft,
						clientY: (parent.clientHeight + self.elem.clientHeight)/2 + parent.offsetTop
					}});
				};
				$zoomRange.on(events);
			}
